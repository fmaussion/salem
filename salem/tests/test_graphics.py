...
freetype_subdir = (
    'freetype_28' if ftver >= Version('2.8.0') else 'freetype_old'
)
...
from salem.graphics import DataLevels, ExtendedNorm, Map, get_cmap, shapefiles
from salem.tests import requires_cartopy, requires_matplotlib
...
baseline_dir = (
    sample_data_dir / 'baseline_images' / baseline_subdir / freetype_subdir
)

tolpy2 = 5 if python_version == 'py3' else 10


def _create_dummy_shp(fname: Path) -> Path:
...
class TestColors(unittest.TestCase):
    @requires_matplotlib
    def test_extendednorm(self) -> None:
        bounds = [1, 2, 3]
        ...
        np.testing.assert_array_equal(
            cmshould([0, 1, 2, 3]), cmshould(mynorm(x))
        )
...
class TestGraphics(unittest.TestCase):
    ...
    def test_datalevels_output(self) -> None:
        ...
        c = graphics.DataLevels(
            nlevels=10, data=np.array([2.5]), vmin=2, vmax=3
        )
        assert_array_equal(c.levels, np.linspace(2, 3, num=10))
        assert c.extend == 'neither'
        c.update({'extend': 'both'})
        assert c.extend == 'both'
        with pytest.raises(AttributeError):
            c.update({'dummy': 't'})

        c = graphics.DataLevels(nlevels=10, data=np.array([0, 9]), vmax=3)
        assert_array_equal(c.levels, np.linspace(0, 3, num=10))
        assert c.extend == 'max'

        c = graphics.DataLevels(nlevels=10, data=np.array([0, 9]), vmin=1)
        ...
        c.set_data(np.array([-12, 8]))
        assert_array_equal(c.vmin, 1)
        assert c.extend == 'min'
        c.set_data(np.array([2, 8]))
        assert c.extend == 'neither'
        c.set_extend('both')
        assert c.extend == 'both'
        c.set_data(np.array([3, 3]))
        assert c.extend == 'both'
        c.set_extend()
        assert c.extend == 'neither'
        ...
        c = graphics.DataLevels(levels=np.array([0, 1, 2]), data=x, cmap=cm)
        r = c.to_rgb()
        assert len(x) == len(r)
        assert c.extend == 'both'
        ...
        x = [0.9, 1.2]
        c = graphics.DataLevels(
            levels=np.array([0, 1, 2]), data=x, cmap=cm, extend='both'
        )
        r = c.to_rgb()
        assert len(x) == len(r)
        assert c.extend == 'both'
        ...
        c = graphics.DataLevels(
            levels=np.array([0, 1, 2]), data=x, cmap=cm, extend='max'
        )
        r = c.to_rgb()
        assert len(x) == len(r)
    ...
    def test_map(self) -> None:
        ...
        c.set_cmap(cmap.name)
        c.set_plot_params(levels=np.array([0, 1, 2, 3]))
        c.set_data(a)
        rgb1 = c.to_rgb()
        c.set_data(a, crs=g)
    ...
    @requires_matplotlib
    def test_increase_coverage(self) -> None:
        ...
        with pytest.raises(
            ValueError, match='Dimensions of data do not match'
        ):
            c.set_data(np.zeros((3, 8)))


@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir, tolerance=10)
def test_extendednorm() -> plt.Figure:
    a = np.zeros((4, 5))
    a[0, 0] = -9999
    a[1, 1] = 1.1
    a[2, 2] = 2.2
    ...
    imax = ax1.imshow(
        a, interpolation='None', norm=norm, cmap=cm, origin='lower'
    )
    ...
    cax = divider.append_axes('right', size='5%', pad=0.2)
    mpl.colorbar.ColorbarBase(cax, extend='both', cmap=cm, norm=norm)
    fig.tight_layout()
    return fig
...
@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir, tolerance=10)
def test_datalevels() -> plt.Figure:
    plt.close()
    ...
    c = DataLevels(levels=np.array([0, 1, 2, 3]), data=a, cmap=cm.name)
    ...
    # Forced bounds
    c = DataLevels(
        levels=np.array([0, 1, 2, 3]), data=a, cmap=cm.name, extend='both'
    )
    ...
    a[0, 0] = -1
    a[3, 3] = 9
    c = DataLevels(nlevels=127, vmin=0, vmax=3, data=a, cmap=cm.name)
...
def test_datalevels_visu_h() -> plt.Figure:
    a = np.array([-1.0, 0.0, 1.1, 1.9, 9.0])
    cm = mpl.colormaps.get_cmap('RdYlBu_r')

    dl = DataLevels(a, cmap=cm.name, levels=np.array([0, 1, 2, 3]))
...
def test_datalevels_visu_v() -> plt.Figure:
    a = np.array([-1.0, 0.0, 1.1, 1.9, 9.0])
    cm = mpl.colormaps.get_cmap('RdYlBu_r')

    dl = DataLevels(
        a.reshape((5, 1)), cmap=cm.name, levels=np.array([0, 1, 2, 3])
    )
...
@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir, tolerance=10)
def test_simple_map() -> plt.Figure:
    a = np.zeros((4, 5))
    a[0, 0] = -1
    a[1, 1] = 1.1
    a[2, 2] = 2.2
    ...
    a[3, 3] = 9
    a_inv = a[::-1, :]
    fs = _create_dummy_shp(Path('fs.shp'))

    # UL Corner
    g1 = Grid(
        nxny=(5, 4), dxdy=(1, -1), x0y0=(-1, 3), proj=wgs84, pixel_ref='corner'
    )
    c1 = Map(g1, ny=4, countries=False)

    # LL Corner
    g2 = Grid(
        nxny=(5, 4), dxdy=(1, 1), x0y0=(-1, -1), proj=wgs84, pixel_ref='corner'
    )
    c2 = Map(g2, ny=4, countries=False)

    # Settings
    for c, data in zip([c1, c2], [a_inv, a]):
        c.set_cmap(mpl.colormaps.get_cmap('jet').name)
        c.set_plot_params(levels=np.array([0, 1, 2, 3]))
        c.set_data(data)
        c.set_shapefile(fs)
...
def test_contourf() -> plt.Figure:
    ...
    c.set_cmap(mpl.colormaps.get_cmap('viridis').name)
    c.set_plot_params(levels=np.array([0, 1, 2, 3]))
    c.set_data(a)
    s = a * 0.0
    s[2, 2] = 1
    c.set_contourf(
        s, interp='linear', hatches=['xxx'], colors='none', levels=[0.5, 1.5]
    )
    ...
    s[0, 4] = 2
    c.set_contour(
        s, interp='linear', colors='k', linewidths=6, levels=[0.5, 1.0, 1.5]
    )
...
@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir)
def test_merca_nolabels() -> plt.Figure:
    grid = mercator_grid(center_ll=(11.38, 47.26), extent=(2000000, 2000000))
...
@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir, tolerance=5)
def test_oceans() -> plt.Figure:
    f = get_demo_file('wrf_tip_d1.nc')
...
@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir)
def test_geometries() -> plt.Figure:
    # UL Corner
    g = Grid(
        nxny=(5, 4),
        dxdy=(10, 10),
    ...
    c = Map(g, ny=4)
    c.set_lonlat_contours(interval=10.0, colors='crimson', linewidths=1)

    c.set_geometry(shpg.Point(10, 10), color='darkred', markersize=60)
    c.set_geometry(
        shpg.Point(5, 5), s=500, marker='s', facecolor='green', hatch='||||'
    )

    s = np.array([(-5, -10), (0.0, -5), (-5, 0.0), (-10, -5)])
...
def test_text() -> plt.Figure:
    ...
    c.set_lonlat_contours(interval=5.0, colors='crimson', linewidths=1)
    ...
    had_c = set()
    for _index, row in shape.iloc[::-1].iterrows():
...
@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir)
def test_hef_linear() -> plt.Figure:
    ...
    c.set_topography(get_demo_file('hef_srtm.tif'), interp='linear')
...
@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir)
def test_hef_default_spline() -> plt.Figure:
    grid = mercator_grid(center_ll=(10.76, 46.798444), extent=(10000, 7000))
    c = Map(grid, countries=False)
...
def test_hef_from_array() -> plt.Figure:
    grid = mercator_grid(center_ll=(10.76, 46.798444), extent=(10000, 7000))
    c = Map(grid, countries=False)
    c.set_lonlat_contours(interval=0)
    c.set_shapefile(get_demo_file('Hintereisferner_UTM.shp'))
...
def test_hef_topo_withnan() -> plt.Figure:
    grid = mercator_grid(center_ll=(10.76, 46.798444), extent=(10000, 7000))
    c = Map(grid, countries=False)
    c.set_lonlat_contours(interval=10)
    c.set_shapefile(get_demo_file('Hintereisferner_UTM.shp'))
...
def test_gmap() -> plt.Figure:
    g = GoogleCenterMap(
        center_ll=(10.762660, 46.794221), zoom=13, size_x=640, size_y=640
    )

    m = Map(g.grid, countries=False, factor=1)
    m.set_lonlat_contours(interval=0.025)
    m.set_shapefile(
        get_demo_file('Hintereisferner.shp'), linewidths=2, edgecolor='darkred'
    )
...
@requires_static_key
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir, tolerance=25)
def test_gmap_transformed() -> plt.Figure:
    dem = GeoTiff(get_demo_file('hef_srtm.tif'))
...
@requires_static_key
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir, tolerance=10)
def test_gmap_llconts() -> plt.Figure:
...
def test_plot_on_map() -> plt.Figure:
    import salem
    from salem.utils import get_demo_file

    ds = salem.open_wrf_dataset(get_demo_file('wrfout_d01.nc'))
    t2_sub = ds.salem.subset(
        corners=((77.0, 20.0), (97.0, 35.0)), crs=salem.wgs84
    ).T2.isel(time=2)
    shdf = salem.read_shapefile(get_demo_file('world_borders.shp'))
    shdf = shdf.loc[
        shdf['CNTRY_NAME'].isin(['Nepal', 'Bhutan'])
...
@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir)
def test_example_docs() -> plt.Figure:
    import salem
    from salem.utils import get_demo_file
    ...
    t2_sub = t2_sub.salem.subset(shape=shdf, margin=2)  # add 2 grid points
    t2_roi = t2_sub.salem.roi(shape=shdf)
    smap = t2_roi.salem.get_map(
        data=t2_roi - 273.15, cmap='RdYlBu_r', vmin=-14, vmax=18
    )
...
def test_colormaps() -> plt.Figure:
    ...
    for ax, cm in zip(axs, ['topo', 'dem', 'nrwc']):
        cb = mpl.colorbar.ColorbarBase(
            ax, cmap=get_cmap(cm), orientation='horizontal'
        )
        cb.set_label(cm)
    return fig
...
@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir, tolerance=5)
def test_geogrid_simulator() -> plt.Figure:
    from salem.wrftools import geogrid_simulator

    g, maps = geogrid_simulator(
        get_demo_file('namelist_mercator.wps'), do_maps=True
    )
    assert len(g) == 4

    fig, axs = plt.subplots(2, 2)
    axs = np.asarray(axs).flatten()
    for _, (m, ax) in enumerate(zip(maps, axs)):
...
@requires_matplotlib
@pytest.mark.mpl_image_compare(baseline_dir=baseline_dir, tolerance=5)
def test_lookup_transform() -> plt.Figure:
    ...
    sm.set_data(out)
    sm.set_geometry(
        dsw.salem.grid.extent_as_polygon(), edgecolor='r', linewidth=2
    )
    sm.visualize(ax=ax)
    return fig
...
@pytest.mark.skip(reason='There is an unknown issue with cartopy')
def test_cartopy_polar() -> plt.Figure:
    import cartopy
