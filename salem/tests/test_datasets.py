...
from salem import Grid, mercator_grid, wgs84, wrftools
...
class TestDataset(unittest.TestCase):
    ...
    def test_subset(self) -> None:
        ...
        g = Grid(nxny=(3, 3), dxdy=(1, 1), x0y0=(0, 0), proj=wgs84)
        d = GeoDataset(g)
        assert isinstance(d, GeoDataset)
        assert g == d.grid
        ...
        with pytest.raises(RuntimeError):
            d.set_subset(corners=((-1, -1), (-1, -1)))
        with pytest.raises(RuntimeError):
            d.set_subset(corners=((5, 5), (5, 5)))
...
class TestGeotiff(unittest.TestCase):
    ...
    def test_subset(self) -> None:
        ...
        go.set_subset(
            corners=((ex[0], ex[2] + eps), (ex[1], ex[3] - eps)),
            crs=wgs84,
            margin=-2,
        )
    ...
    def test_xarray(self) -> None:
        ...
        np.testing.assert_array_equal(
            ref.shape, (gos.salem.grid.ny, gos.salem.grid.nx)
        )
        np.testing.assert_array_equal(ref.shape, totest.shape)
        np.testing.assert_array_equal(ref, totest)
...
class TestGeoNetcdf(unittest.TestCase):
    ...
    def test_wrf(self) -> None:
        ...
        for i in ['1', '2']:
            fw = get_demo_file('wrf_tip_d{}.nc'.format(i))
            d = GeoNetcdf(fw)
            assert isinstance(d, GeoDataset)
        ...
        # Auto dimensions
        assert d1.t_dim == 'Time'
        assert d1.x_dim == 'west_east'
        assert d1.y_dim == 'south_north'
        assert d1.z_dim is None
        ...
        assert d1.grid.nx * 3 == d2.grid.nx
        assert d1.grid.ny * 3 == d2.grid.ny
        assert np.min(d1.roi) == 1
    ...
    def test_longtime(self) -> None:
        """There was a bug with time"""

        fs = get_demo_file('test_longtime.nc')
        c = GeoNetcdf(fs)
        assert len(c.time) == 2424
        assert_array_equal(
            c.time[0:2],
            pd.to_datetime([datetime(1801, 10, 1), datetime(1801, 11, 1)]),
        )

    def test_diagnostic_vars(self) -> None:
        d = WRF(get_demo_file('wrf_tip_d1.nc'))
        d2 = GeoNetcdf(get_demo_file('wrf_tip_d2.nc'))
        assert 'T2C' in d.variables
        ...
        assert tot.shape[-1] * 3 == d2.grid.nx
        assert tot.shape[-2] * 3 == d2.grid.ny
        np.testing.assert_allclose(ref, tot)
...
class TestGoogleStaticMap(unittest.TestCase):
    ...
    def test_center(self) -> None:
        ...
        gm = GoogleCenterMap(
            center_ll=(10.762660, 46.794221), zoom=13, size_x=500, size_y=500
        )
        gm.set_roi(shape=get_demo_file('Hintereisferner.shp'))
        gm.set_subset(toroi=True, margin=10)
        img = gm.get_vardata()[..., :3]
    ...
    def test_visible(self) -> None:
        ...
        x = np.array([91.176036, 92.05, 88.880927])
        y = np.array([29.649702, 31.483333, 29.264956])

        g = GoogleVisibleMap(
            x=x, y=y, size_x=400, size_y=400, maptype='terrain'
        )
        img = g.get_vardata()[..., :3]
        ...
        with pytest.raises(ValueError):
            GoogleVisibleMap(x=x, y=y, zoom=12)
        ...
        gm2 = GoogleVisibleMap(
            x=i, y=j, crs=d.grid, scale=2, size_x=500, size_y=500
        )
        assert (gm.grid.nx * 2) == gm2.grid.nx
        assert gm.grid.extent == gm2.grid.extent

        # Test regression for non array inputs
        grid = mercator_grid(center_ll=(72.5, 30.0), extent=(2.0e6, 2.0e6))
        GoogleVisibleMap(
            x=np.array([0, grid.nx - 1]),
            y=np.array([0, grid.ny - 1]),
            crs=grid,
        )
...
class TestWRF(unittest.TestCase):
    def test_unstagger(self) -> None:
        ...
        with netCDF4.Dataset(wf) as nc:
            ...
            assert_allclose(
                v[0:2, 2:12, 5:10, 15:17], ref[0:2, 2:12, 5:10, 15:17]
            )
            assert_allclose(v[1:2, 2:, 5:10, 15:17], ref[1:2, 2:, 5:10, 15:17])
            assert_allclose(
                v[1:2, :-2, 5:10, 15:17], ref[1:2, :-2, 5:10, 15:17]
            )
            assert_allclose(
                v[1:2, 2:-4, 5:10, 15:17], ref[1:2, 2:-4, 5:10, 15:17]
            )
            ...
            # Under WRF
            nc_wrf = WRF(wf)
            assert_allclose(nc_wrf.get_vardata('PH'), ref)
            nc_wrf.set_period(1, 2)
            assert_allclose(nc_wrf.get_vardata('PH'), ref[1:3, ...])
    ...
    def test_unstagger_compressed(self) -> None:
        ...
        ncc = WRF(wfc)
        assert_allclose(
            nc.get_vardata('PH'), ncc.get_vardata('PH'), rtol=0.003
        )
        nc.set_period(1, 2)
        ncc.set_period(1, 2)
        assert_allclose(
    ...
    def test_staggeredcoords(self) -> None:
        wf = get_demo_file('wrf_cropped.nc')
        nc = GeoNetcdf(wf)
        lon, lat = nc.grid.xstagg_ll_coordinates
        assert_allclose(
            np.squeeze(nc.variables['XLONG_U'][0, ...]), lon, atol=1e-4
        )
        assert_allclose(
            np.squeeze(nc.variables['XLAT_U'][0, ...]), lat, atol=1e-4
        )
        lon, lat = nc.grid.ystagg_ll_coordinates
        assert_allclose(
            np.squeeze(nc.variables['XLONG_V'][0, ...]), lon, atol=1e-4
        )
        assert_allclose(
            np.squeeze(nc.variables['XLAT_V'][0, ...]), lat, atol=1e-4
        )

    def test_staggeredcoords_compressed(self) -> None:
        wf = get_demo_file('wrf_cropped_compressed.nc')
        nc = GeoNetcdf(wf)
        lon, lat = nc.grid.xstagg_ll_coordinates
        assert_allclose(
